 ...
 * Mouse-bound attack/use are polled every frame; keyboard-bound ones are
 * counted from key events (see KeyboardHandlerMixin).
 */
public class KeyTracker {
    ...
    /** Called from KeyboardHandlerMixin for every keyboard event. */
    public static void onKeyEvent(int key, int action) {
        if (action == GLFW.GLFW_PRESS || action == GLFW.GLFW_REPEAT) {
            Options o = Minecraft.getInstance().options;
            if (o != null) {
                InputConstants.Key atk = boundKey(o.keyAttack);
                if (atk.getType() != InputConstants.Type.MOUSE && atk.getValue() == key) {
                    record(attackTs);
                }

                InputConstants.Key use = boundKey(o.keyUse);
                if (use.getType() != InputConstants.Type.MOUSE && use.getValue() == key) {
                    record(useTs);
                }
            }
        }
    ...
    /** The GLFW window handle (Window.getWindow() in this Minecraft version). */
